...
def build_radar(history,seed=None,auto=None,action_map=None,backtest_stats=None,walk_forward_stats=None,news_map=None,news_available=False,
    ...
    for _,r in latest_all.iterrows():
        ...
        for cat in ["SWING","SHORT","LONG"]:
            ...
            ov=combine([(ts,.55),(ms,.35),(fs,.10)]) if cat=="SWING" else (
            ...
            if eligible:
                if cat=="SWING":
                    ...
                    if ov>=threshold and ts>=70 and lv["rr"]>=1.7 and not blocked and not news_blocked:
                        rec="BUY" if (pd.notna(bt_sample) and bt_sample>=30 and pd.notna(bt_wr) and bt_wr>=52) else "WATCH"
                    elif ov>=64 and not blocked and not news_blocked:rec="WATCH"
                elif cat=="SHORT":
                    if ov>=75 and ts>=64 and lv["rr"]>=1.6 and not blocked and not news_blocked:
                        rec="BUY" if (pd.notna(bt_sample) and bt_sample>=30 and pd.notna(bt_wr) and bt_wr>=52) else "WATCH"
                    elif ov>=62 and not blocked and not news_blocked:rec="WATCH"
                else:
                    if not fresh or pd.isna(fs):rec="WATCH" if ov>=60 and not blocked and not news_blocked else "AVOID"
                    elif ov>=78 and fs>=72 and not blocked and not news_blocked:rec="BUY"
                    elif ov>=62 and not blocked and not news_blocked:rec="WATCH"
            ...
            strong=(eligible and rec=="BUY" and ov>=strong_cfg.get("overall",85) and ts>=strong_cfg.get("technical",80) and
                    ...
                    regime!="WEAK" and not blocked and act.get("severity","NONE") not in ("REVIEW","BLOCK") and news_status=="PASS" and entry_status=="ENTRY VALID" and conf=="HIGH")
            ...
            if blocked or news_blocked or not eligible:signal="AVOID"
            rows.append({
              ...
              "Recommendation":rec,"Signal":signal,"EntryStatus":entry_status,"Overall":round(ov,1),"Technical":round(ts,1),"Momentum":round(ms,1),
    ...
    for sym,g in short_hist.items():
        ...
        for cat in ["SWING","SHORT","LONG"]:
            rows.append({
                ...
                "Overall":np.nan,"Technical":np.nan,"Momentum":np.nan,"Fundamental":np.nan,"FundamentalFresh":"NO","FundamentalCompleteness%":0,
