...
def _stock_candidates(radar: pd.DataFrame, horizon_days: int):
    ...
    for _,r in x.iterrows():
        ...
        out.append(_candidate(
            ...
            Source="NSE EOD + local validation + market intelligence",ProductKey=f"STOCK:{_txt(r.get('Symbol'))}",Conditional=timing!="INVEST NOW",
            MarketIntelligenceScore=_num(r.get("MarketIntelligenceScore"),np.nan),MarketRegime=_txt(r.get("InstitutionalBias"),"UNAVAILABLE")+" / "+_txt(r.get("BreadthBias"),"UNAVAILABLE"),
            MarketContext=_txt(r.get("MarketContext"))
        ))
    return out
...
def build_plan(candidates: pd.DataFrame, amount: float, horizon_days: int, risk_profile: str, min_reserve_pct=None, essential_money=False, monthly=False, market_context=None):
    amount=float(amount)
    ...
    if min_reserve_pct is not None:reserve=max(reserve,float(min_reserve_pct))
    # In a risk-off market, preserve more optional liquidity rather than forcing deployment.
    if isinstance(market_context,dict) and str(market_context.get("Regime",""))=="RISK-OFF":
        reserve=max(reserve,20 if horizon_days<=365 else 12)
    reserve=min(max(reserve,0),80)
    ...
    if isinstance(market_context,dict) and str(market_context.get("Regime",""))=="RISK-OFF" and action=="INVEST NOW WITH PLAN":
        action="STAGGER / KEEP RESERVE"
    summary={"AmountRs":round(amount,2),"HorizonDays":int(horizon_days),"Horizon":human_horizon(horizon_days),"Risk":risk_profile,"PlanAction":action,"ReserveRs":round(reserve_amt,2),"DownsideValueRs":round(amount+down,2) if vd else np.nan,"BaseValueRs":round(amount+base,2) if vb else np.nan,"UpsideValueRs":round(amount+up,2) if vu else np.nan,"BaseGainRs":round(base,2) if vb else np.nan,"ReviewDate":str(date.today()+timedelta(days=max(14,min(90,horizon_days//4 or 14)))),
             "MarketRegime":str((market_context or {}).get("Regime","UNAVAILABLE")) if isinstance(market_context,dict) else "UNAVAILABLE",
             "MarketIntelligenceScore":_num((market_context or {}).get("Score"),np.nan) if isinstance(market_context,dict) else np.nan}
    if isinstance(market_context,dict) and market_context.get("Regime") not in (None,"","UNAVAILABLE"):
        why.append(f"Current market-intelligence regime is {market_context.get('Regime')} ({market_context.get('Score','—')}/100); this can change timing/reserve but cannot override product-specific quality gates.")
...
def optimize_money(amount: float, horizon_days: int, risk_profile: str, radar=None, mf=None, bonds=None, ipos=None, opts=None, history=None, cfg=None, metal_state=None, crypto_loader=None, existing_exposure=None, include_existing=True, exclusions=None, min_reserve_pct=None, essential_money=False, monthly=False, market_context=None):
    ...
    plan = build_plan(
        ...
        essential_money=essential_money, monthly=monthly, market_context=market_context,
    )
