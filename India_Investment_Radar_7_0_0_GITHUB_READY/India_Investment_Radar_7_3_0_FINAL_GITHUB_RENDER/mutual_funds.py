...
def _parse_amfi_nav_text(text):
    """Parse both AMFI NAV formats.

    AMFI introduced a new Complete NAV format in 2026 with separate Plan and Option
    columns (8 fields). The older 6-field format remains temporarily available. This
    parser intentionally supports both so the complete scheme/plan universe is not
    silently lost when AMFI changes format.
    """
    rows=[]; scheme_type=''; category=''; fund_house=''; current_format='unknown'
    for raw in str(text).splitlines():
        line=raw.strip().lstrip('\ufeff')
        if not line:continue
        low=line.lower()
        if low.startswith('scheme code;'):
            current_format='new' if 'plan;' in low and 'option;' in low else 'old'
            continue
        if ';' not in line:
            up=line.upper()
            if 'SCHEMES' in up and ('OPEN ENDED' in up or 'CLOSE ENDED' in up or 'CLOSED ENDED' in up or 'INTERVAL' in up):
                scheme_type=line.split('(')[0].strip(' -')
                m=re.search(r'\((.*?)\)',line)
                category=m.group(1).strip() if m else line
            elif 'MUTUAL FUND' in up and len(line)<180:
                fund_house=line
            continue
        ...
        if len(parts)>=8:  # AMFI current format: Code;ISIN1;ISIN2;Name;Plan;Option;NAV;Date
            name=parts[3].strip();plan=parts[4].strip() or _plan(name);option=parts[5].strip() or _option(name)
            nav=pd.to_numeric(parts[6],errors='coerce');dt=parts[7].strip();src='AMFI Complete NAV — current format'
            plan='Direct' if 'DIRECT' in plan.upper() else ('Regular' if 'REGULAR' in plan.upper() else (plan or 'Unspecified'))
            ou=option.upper()
            if 'GROWTH' in ou:option='Growth'
            elif 'IDCW' in ou or 'DIVIDEND' in ou or 'DCW' in ou:option='IDCW'
            elif not option:option='Other/Unspecified'
        else:  # AMFI legacy format: Code;ISIN1;ISIN2;Name;NAV;Date
            name=parts[3].strip();nav=pd.to_numeric(parts[4],errors='coerce');dt=parts[5].strip();plan=_plan(name);option=_option(name);src='AMFI Complete NAV — legacy format'
        rows.append({'SchemeCode':code,'SchemeName':name,'NAV':float(nav) if pd.notna(nav) else np.nan,'NAVDate':dt,
                     ...
                     'Plan':plan,'Option':option,'ISIN1':parts[1] if len(parts)>1 else '',
                     'ISIN2':parts[2] if len(parts)>2 else '','Source':src})
    df=pd.DataFrame(rows,columns=UNIVERSE_COLUMNS)
    if not df.empty:
        # SchemeCode is plan/option specific in the current AMFI feed; retain every unique code.
        df=df.drop_duplicates(['SchemeCode'],keep='last').reset_index(drop=True)
    return df
